...
def parse_tasks(file_path, table, registered_vars=None):
    ...
    for line in lines:
        ...
        if register:
            clean_var = register.group().replace("register:", "").strip()
            registered_vars.append(clean_var)
        else:
            match_obj = re.findall("{{[A-Za-z0-9 -_.]*}}", line)
            if match_obj:
                for match in match_obj:
                    clean_var = parse_used_variable(match)
                    if clean_var and clean_var not in registered_vars and clean_var not in table:
                        table[clean_var] = Entry(clean_var, "Yes", "-")
